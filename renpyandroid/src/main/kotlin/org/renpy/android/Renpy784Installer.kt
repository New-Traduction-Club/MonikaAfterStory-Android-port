...
object Renpy784Installer {

    private const val TAG = "Renpy784Installer"
    private const val VERSION = "7.8.4"

    fun isInstalled(gameDir: File): Boolean {
        val versionFile = File(gameDir, ".runtime_784.version")
    ...
    fun ensureInstalled(context: Context, gameDir: File): Boolean {
        if (isInstalled(gameDir)) {
            Log.i(TAG, "Ren'Py 7.8.4 runtime already installed in ${gameDir.name}")
            return true
        }

        Log.i(TAG, "Installing Ren'Py 7.8.4 runtime in ${gameDir.name}...")

        try {
            cleanOldEngineFiles(gameDir)

            val privateOk = extractTarGz(context, "renpy784/private.mp3", gameDir)
            if (!privateOk) {
                Log.e(TAG, "Failed to extract renpy784/private.mp3")
                return false
            }

            val commonDir = File(gameDir, "renpy/common")
            commonDir.mkdirs()
            val commonOk = extractTarGz(context, "renpy784/common.mp3", commonDir)
            if (!commonOk) {
                Log.e(TAG, "Failed to extract renpy784/common.mp3")
                return false
            }

            cleanForeignBinaries(gameDir)

            File(gameDir, ".runtime_784.version").writeText(VERSION)
            File(gameDir, ".nomedia").createNewFile()

            Log.i(TAG, "Ren'Py 7.8.4 runtime successfully installed in ${gameDir.name}")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "Error installing Ren'Py 7.8.4 runtime", e)
            return false
        }
    }
