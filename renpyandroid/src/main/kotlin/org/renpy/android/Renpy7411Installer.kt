...
object Renpy7411Installer {

    private const val TAG = "Renpy7411Installer"
    private const val VERSION = "7.4.11"

    fun isInstalled(gameDir: File): Boolean {
        val versionFile = File(gameDir, ".runtime_7411.version")
    ...
    fun ensureInstalled(context: Context, gameDir: File): Boolean {
        if (isInstalled(gameDir)) {
            Log.i(TAG, "Ren'Py 7.4.11 runtime already installed in ${gameDir.name}")
            return true
        }

        Log.i(TAG, "Installing Ren'Py 7.4.11 runtime in ${gameDir.name}...")

        try {
            cleanOldEngineFiles(gameDir)

            val privateOk = extractTarGz(context, "renpy7411/private.mp3", gameDir)
            if (!privateOk) {
                Log.e(TAG, "Failed to extract renpy7411/private.mp3")
                return false
            }

            val commonDir = File(gameDir, "renpy/common")
            commonDir.mkdirs()
            val commonOk = extractTarGz(context, "renpy7411/common.mp3", commonDir)
            if (!commonOk) {
                Log.e(TAG, "Failed to extract renpy7411/common.mp3")
                return false
            }

            cleanForeignBinaries(gameDir)

            File(gameDir, ".runtime_7411.version").writeText(VERSION)
            File(gameDir, ".nomedia").createNewFile()

            Log.i(TAG, "Ren'Py 7.4.11 runtime successfully installed in ${gameDir.name}")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "Error installing Ren'Py 7.4.11 runtime", e)
            return false
        }
    }
