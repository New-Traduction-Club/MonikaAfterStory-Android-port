...
object Renpy853Installer {

    private const val TAG = "Renpy853Installer"
    const val VERSION = "8.5.3"

    fun isInstalled(gameDir: File): Boolean {
        val versionFile = File(gameDir, ".runtime_853.version")
    ...
    fun ensureInstalled(context: Context, gameDir: File): Boolean {
        if (isInstalled(gameDir)) {
            Log.i(TAG, "Ren'Py 8.5.3 runtime already installed in ${gameDir.name}")
            return true
        }

        Log.i(TAG, "Installing Ren'Py 8.5.3 runtime in ${gameDir.name}...")

        try {
            cleanOldEngineFiles(gameDir)

            val privateOk = extractTarGz(context, "renpy853/private.mp3", gameDir)
            if (!privateOk) {
                Log.e(TAG, "Failed to extract renpy853/private.mp3")
                return false
            }

            val commonDir = File(gameDir, "renpy/common")
            commonDir.mkdirs()
            val commonOk = extractTarGz(context, "renpy853/common.mp3", commonDir)
            if (!commonOk) {
                Log.e(TAG, "Failed to extract renpy853/common.mp3")
                return false
            }

            cleanForeignBinaries(gameDir)

            File(gameDir, ".runtime_853.version").writeText(VERSION)
            File(gameDir, ".nomedia").createNewFile()

            Log.i(TAG, "Ren'Py 8.5.3 runtime successfully installed in ${gameDir.name}")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "Error installing Ren'Py 8.5.3 runtime", e)
            return false
        }
    }
