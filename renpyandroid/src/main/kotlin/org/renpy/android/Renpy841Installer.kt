...
object Renpy841Installer {

    private const val TAG = "Renpy841Installer"
    const val VERSION = "8.4.1"

    fun isInstalled(gameDir: File): Boolean {
        val versionFile = File(gameDir, ".runtime_841.version")
    ...
    fun ensureInstalled(context: Context, gameDir: File): Boolean {
        if (isInstalled(gameDir)) {
            Log.i(TAG, "Ren'Py 8.4.1 runtime already installed in ${gameDir.name}")
            return true
        }

        Log.i(TAG, "Installing Ren'Py 8.4.1 runtime in ${gameDir.name}...")

        try {
            cleanOldEngineFiles(gameDir)

            val privateOk = extractTarGz(context, "renpy841/private.mp3", gameDir)
            if (!privateOk) {
                Log.e(TAG, "Failed to extract renpy841/private.mp3")
                return false
            }

            val commonDir = File(gameDir, "renpy/common")
            commonDir.mkdirs()
            val commonOk = extractTarGz(context, "renpy841/common.mp3", commonDir)
            if (!commonOk) {
                Log.e(TAG, "Failed to extract renpy841/common.mp3")
                return false
            }

            cleanForeignBinaries(gameDir)

            File(gameDir, ".runtime_841.version").writeText(VERSION)
            File(gameDir, ".nomedia").createNewFile()

            Log.i(TAG, "Ren'Py 8.4.1 runtime successfully installed in ${gameDir.name}")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "Error installing Ren'Py 8.4.1 runtime", e)
            return false
        }
    }
