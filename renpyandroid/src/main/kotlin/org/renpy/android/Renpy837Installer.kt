...
object Renpy837Installer {

    private const val TAG = "Renpy837Installer"
    private const val VERSION = "8.3.7"

    fun isInstalled(gameDir: File): Boolean {
        val versionFile = File(gameDir, ".runtime_837.version")
    ...
    fun ensureInstalled(context: Context, gameDir: File): Boolean {
        if (isInstalled(gameDir)) {
            Log.i(TAG, "Ren'Py 8.3.7 runtime already installed in ${gameDir.name}")
            return true
        }

        Log.i(TAG, "Installing Ren'Py 8.3.7 runtime in ${gameDir.name}...")

        try {
            cleanOldEngineFiles(gameDir)

            val privateOk = extractTarGz(context, "renpy837/private.mp3", gameDir)
            if (!privateOk) {
                Log.e(TAG, "Failed to extract renpy837/private.mp3")
                return false
            }

            val commonDir = File(gameDir, "renpy/common")
            commonDir.mkdirs()
            val commonOk = extractTarGz(context, "renpy837/common.mp3", commonDir)
            if (!commonOk) {
                Log.e(TAG, "Failed to extract renpy837/common.mp3")
                return false
            }

            cleanForeignBinaries(gameDir)

            File(gameDir, ".runtime_837.version").writeText(VERSION)
            File(gameDir, ".nomedia").createNewFile()

            Log.i(TAG, "Ren'Py 8.3.7 runtime successfully installed in ${gameDir.name}")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "Error installing Ren'Py 8.3.7 runtime", e)
            return false
        }
    }
